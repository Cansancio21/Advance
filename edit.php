...
session_start();
include 'db.php'; 
include 'EditClass.php'; // Include the DataEditor class
...
$id = $_GET['id'];
$dataEditor = new edit($conn, $id);

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $dataEditor->updateData($_POST);
    header("Location: submit.php"); // Redirect to submit.php after updating
    exit();
}

$formationData = $dataEditor->getFormationData();
$birthData = $dataEditor->getBirthData();
$addressData = $dataEditor->getAddressData();
$contactData = $dataEditor->getContactData();
$parentsData = $dataEditor->getParentsData();
$dataEditor->closeConnection();
?>
            ...
            <div class="details personal">
                ...
                <div class="country">
                    ...
                    <select name="birthcountry" id="birthcountry" required>
                        ...
                        foreach ($countries as $country) {
                            echo "<option value=\"$country\" " . ($birthData['b_country'] == $country ? 'selected' : '') . ">$country</option>";
                        }
                        ?>
                    </select>
                </div>
                ...
                <div class="country">
                    ...
                    <select name="country" id="country" required>
                        ...
                        <?php
                        foreach ($countries as $country) {
                            echo "<option value=\"$country\" " . ($addressData['h_country'] == $country ? 'selected' : '') . ">$country</option>";
                        }
                        ?>
                    </select>
                </div>
